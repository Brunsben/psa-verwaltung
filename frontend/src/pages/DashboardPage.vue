  ...
  <div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      <div @click="page = 'kameraden'"
        class="min-w-0 bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 flex items-center justify-between gap-2 cursor-pointer hover:shadow-md hover:border-red-200 dark:hover:border-red-800 transition-all">
        <div class="min-w-0">
          <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ stats.kameraden }}</div>
          <div class="text-xs font-medium text-gray-500 dark:text-gray-400 mt-0.5 break-words">Kameraden aktiv</div>
        </div>
      ...
      <div @click="page = 'ausruestung'; filterStatus = ''"
        class="min-w-0 bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 flex items-center justify-between gap-2 cursor-pointer hover:shadow-md hover:border-blue-200 dark:hover:border-blue-800 transition-all">
        <div class="min-w-0">
          <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ stats.ausruestung }}</div>
          <div class="text-xs font-medium text-gray-500 dark:text-gray-400 mt-0.5 break-words">Ausrüstungsstücke</div>
        </div>
      ...
      <div @click="page = 'ausruestung'; filterStatus = 'Prüfung fällig'"
        class="min-w-0 bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 flex items-center justify-between gap-2 cursor-pointer hover:shadow-md hover:border-orange-200 dark:hover:border-orange-800 transition-all">
        <div class="min-w-0">
          <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ stats.pruefungFaellig }}</div>
          <div class="text-xs font-medium text-gray-500 dark:text-gray-400 mt-0.5 break-words">Prüfungen ≤ 30 Tage</div>
        </div>
      ...
      <div @click="page = 'warnungen'"
        :class="['min-w-0 rounded-xl p-4 shadow-sm border flex items-center justify-between gap-2 cursor-pointer hover:shadow-md transition-all',
        ...
        <div class="min-w-0">
          ...
          <div class="text-xs font-medium text-gray-500 dark:text-gray-400 mt-0.5 break-words">Aktive Warnungen</div>
        </div>

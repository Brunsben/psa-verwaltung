<template>
  <div>
    ...
    <div class="md:hidden space-y-2">
      <div v-if="!benutzer.length" class="text-center text-gray-400 dark:text-gray-500 text-sm py-8">Keine Benutzer vorhanden</div>
      <div v-for="b in benutzer" :key="b.Id"
        ...
        <div class="flex items-start justify-between gap-2">
          <div class="min-w-0 flex-1">
            <div class="font-semibold text-gray-900 dark:text-white">
              {{ b.Benutzername }}
              <span v-if="b.Id === currentUser?.Id" class="text-xs text-gray-400 font-normal ml-1">(du)</span>
            </div>
        ...
        <dl class="mt-2.5 grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
          <div>
            <dt class="text-gray-400 dark:text-gray-500">Kamerad</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ b.KameradId ? (kameraden.find(k => k.Id == b.KameradId)?.Vorname + ' ' + kameraden.find(k => k.Id == b.KameradId)?.Name || 'ID: ' + b.KameradId) : '–' }}</dd>
          </div>
          <div>
            <dt class="text-gray-400 dark:text-gray-500">Status</dt>
            <dd>
              <span :class="['px-1.5 py-0.5 rounded-full text-xs font-semibold', b.Aktiv !== false ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-500']">
